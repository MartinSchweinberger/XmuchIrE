...
check <- stringr::str_replace_all(cfiles, ".*_(.*?)_.*", "\\1")
# load texts
...
```{r xm_2_05, eval=T}
# simplify names
names(texts) <- stringr::str_replace_all(names(texts), ".*/(.*?).txt", "\\1")
...
# save data
base::saveRDS(kwic, file = here::here("data", "kwics.rda"))
# load data
...
```{r}
clean_kwic1 <- kwics %>%
  dplyr::mutate(variety = stringr::str_replace_all(docname, ".*_(.*?)_.*", "\\1"),
